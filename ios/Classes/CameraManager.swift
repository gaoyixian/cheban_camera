    ...
    open func capturePictureDataWithCompletion(_ imageCompletion: @escaping (CaptureResult) -> Void) {
        ...
        sessionQueue.async {
            ...
            if let connection = stillImageOutput.connection(with: AVMediaType.video),
                ...
                if self.cameraDevice == CameraDevice.front,
...
//    open func fileOutput(_: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from _: [AVCaptureConnection], error: NSError?) {
//        if let error = error {
...
//    }
    ...
    fileprivate func _createMovieOutput() {
        ...
        // TODO: 给后面开发的人提个醒。。。这玩意竟然在cameraManager给写死了，调整最大时间的时候要注意下
        newMovieOutput.maxRecordedDuration = CMTime(seconds: 30.4, preferredTimescale: 100000)
